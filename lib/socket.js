...
io.sockets.on('connection', function (socket) {
      ...
      try {
        ...
        if (_.isEmpty(ret)) {
          ...
          if (onlineTime >= (60 * 6 * 50) / GRADETIMES) { //加分上限
            ...
            await addUserGrade(userId, 50)
          } else {
            let count = Math.floor(onlineTime / (60 * 6))
            let addGrade = count * GRADETIMES
            record.set('addGrade', addGrade)
            await addUserGrade(userId, addGrade)
          }
          await record.save()
        } else {
          let addGrade
          let record = AV.Object.createWithoutData('OnlineTime', ret.get('objectId'))
          onlineTime += ret.get('onlineTime')
          if (onlineTime >= (60 * 30)) {
            let mission = new Mission()
            let query = new AV.Query('Invitation')
            query.include('inviter')
          ...
          if (onlineTime >= (60 * 6 * 50) / GRADETIMES) { //加分上限
            ...
            let count = Math.floor(onlineTime / (60 * 6)) //有多少个6分钟
            addGrade = count * GRADETIMES
            record.set('addGrade', addGrade)
            addGrade -= ret.get('addGrade')
          }
          record.set('onlineTime', onlineTime)
          await addUserGrade(userId, addGrade)
          await record.save()
  ...
  const addUserGrade = (userId, grade) => {
    return new Promise(async (resolve, reject) => {
      try {
        let query = new AV.Query(`_User`)
        query.equalTo('objectId', userId)
        let User = await query.first()
        let user = AV.Object.createWithoutData('_User', userId)
        user.set('grade', User.get('grade') + grade)
        await user.save()
        if(grade > 0){
          let g = new Grade()
          await g.recordGrade(userId, `在线时长奖励`, grade, `+`, User.get('grade') + grade)
        }
        resolve()
      } catch (err) {
