...
io.sockets.on('connection', function (socket) {
  ...
  socket.on('login', async(userId) => {
    console.log(`${userId}login${new Date()}`)
    if (userId) {
    ...
    try {
      ...
      await setTimeout(async() => {
        ...
        if (_.isEmpty(result2)) {
          ...
          let roomInfo = await roomObj.userRoom(userId)
          let ownerRoom = await new Room(roomInfo.data.roomId)
          if (room !== `no room` || roomInfo.status !== 203) {
            console.log(`userLeave ----> userId is ${userId}`)
            let roomObj = await new Room(roomInfo.data.roomId)
            if (userId === ownerRoom.owner.userId) { //退出的是房主
              console.log(`owner leave successfully`)
              await ownerRoom.ownerOffline()
              socket.in(`room${roomInfo.data.roomId}`).emit('ownerLeaveRoom', {
                roomId: roomInfo.data.roomId,
                userId: userId
              })
              let socketRet = await roomObj.deleteUser(userId)
              if (socketRet) { //退出房间成功
              } else { //退出的不是房主
                socket.to(`room${roomInfo.data.roomId}`).emit('userLeaveRoom', {
                  roomId: roomInfo.data.roomId,
                  userId: userId
                })
              }
            }
            let roomUsers = await userLeave(userId, room)
            console.log(`roomUsers is `, JSON.stringify(roomUsers))
          }
        }
      }, 1000)
    } catch (err) {
