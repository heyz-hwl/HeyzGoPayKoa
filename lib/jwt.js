...
const sign = async(ctx, next) => {
  try {
    let userId = (ctx.request.body && ctx.request.body.userId) || (ctx.query && ctx.query.userId) || ctx.headers['x-user-id'];
    ...
    if (userId) { //判断用户是否存在
      ...
      if (ret) { //如果存在该用户
        ...
        console.log(`sign token is ${token}`)
        ctx.locals = {}
...
//用于 mocha test
const createToken = (anotherUserId) => {
  return new Promise(async(resolve, reject) => {
    try {
      let userId = anotherUserId || '59929e68a22b9d0057108c6f'
      let ret = await getUserInfo(userId)
      let token = await jwt.sign({
        userId: userId
      }, config.jwt.secret, {
  ...
  try {
    ...
    if (token) {
      ...
      if (Math.floor(Date.now() / 1000) <= decoded.payload.exp + config.jwt.allowExpiresIn) {
        ...
        let ret = await getUserInfo(userId)
        if (ret) {
          let newToken = await refreshToken(token, config.jwt.expiresIn, config.jwt.secret)
