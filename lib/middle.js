...
const recordHeyzNum = (ctx, next) => {
  return new Promise(async(resolve, reject) => {
    try {
      let userId = ctx.decode.userId; //获取用户ID
      let heyzNum = Number(ctx.req.body.heyzNum);
      let orderNo = moment().format('YYYYMMDDHHmmss') + util.randomNum(4); //时分秒+4位随机数，组成订单号
      let orderName = '埋宝箱';
      let amount = util.oprate(heyzNum, config.rate, 'div');
      let time = moment().format('YYYY-MM-DD HH:mm:ss');
      let sql = `insert into Consume values(null, "${userId}", "${orderNo}", "${orderName}", "${amount}", "${heyzNum}", "${time}", "${config.rate}");`;
      let info = await db.excute(sql)
      console.log('recordHeyzNum insert success');
      resolve();
    } catch (err) {
      reject(`recordHeyzNum err-->${err}`)
    }
  })
}
...
const getPageInfoByMySql = (tableName, userId, page, size) => {
  return new Promise(async(resolve, reject) => {
    try {
      ...
      let childSql = 'select id from ' + tableName + ' where 1 = 1';
      //如果存在userId
      if (userId) {
      ...
      reject(`getPageInfoByMySql err ->${err}`)
    }
  })
}
...
//记录钱包黑石数的使用情况
const walletRecord = (info) => {
  return new Promise(async(resolve, reject) => {
    try {
      let sql = `insert into TradeRecord values(null, "${info.userId}", "${info.type}", "${info.status}", "${info.yuyi_num}", "${info.time}", "${info.timeStamp}")`;
      let result = await db.excute(sql)
      resolve(result);
    } catch (err) {
      reject(`walletRecord err ->${err}`)
    }
  })
}
...
function push(toUserId, data) {
  return new Promise(function (resolve, reject) {
    ...
    AV.Push.send({
      channels: toUserId,
      prod: config.pushConfig.PROFILE,
      data: data
    }).then(() => {
      resolve();
    }).catch(err => {
